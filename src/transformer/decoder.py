...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, h: int):
        super().__init__()
        self.masked_multi_head_attention = MultiHeadAttention(d_model=d_model, h=h, masking=True)
    ...
    def forward(self, x: torch.Tensor, enc_emb: torch.Tensor) -> torch.Tensor:
        att_out = self.masked_multi_head_attention(x)
        x = self.add_norm_1(x, att_out)

        att_out = self.multi_head_attention(enc_emb)
